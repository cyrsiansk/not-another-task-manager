...
def create_task(
    db: Session, user_id: UUID, title: str, description: Optional[str] = None
) -> Task:
...
def list_tasks(
    db: Session, user_id: UUID, status: Optional[TaskStatus] = None, limit: int = 50
) -> List[Task]:
...
def soft_delete_task(db: Session, task_id: UUID, user_id: UUID) -> bool:
    stmt = (
        update(Task)
